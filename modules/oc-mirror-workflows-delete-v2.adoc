...
The delete feature in oc-mirror plugin v2 may not work as expected due to a known issue in the distribution registry. This issue does not occur when using {quay}.

To work around this issue, perform one of the following actions:

* Restarting the registry container might temporarily resolve the issue.
[source,terminal]
----
$ podman restart <registry_container>
----

* Disable caching in the registry configuration: 
** Modify the `/etc/docker/registry/config.yml` file to disable the `blobdescriptor` cache.

...
.. Open the `working-dir/logs` file located in the oc-mirror output directory.
...
. Errors related to generated cluster resources. Even if some images fail to mirror, oc-mirror v2 will still generate cluster resources such as `IDMS.yaml` and `ITMS.yaml` files, for the successfully mirrored images.

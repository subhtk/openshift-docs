...
As part of mirroring images from Red Hat to your mirror registry, you must create a container image registry credentials file.

ifdef::restricted[]
[WARNING]
====
Do not use this image registry credentials file as the pull secret when installing a cluster. Using this file grants write access to your mirror registry for all machines in the cluster.
====
endif::restricted[]
...
ifdef::oc-mirror[]
. Save the file either as either `~/.docker/config.json` or `$XDG_RUNTIME_DIR/containers/auth.json`.
endif::[]
...
file and add a section that describes your registry:
+
[source,json]
----
ifndef::openshift-origin[]
